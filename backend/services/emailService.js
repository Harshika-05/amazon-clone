/**
 * Email Service — Resend (HTTP API over port 443)
 * 
 * WHY NOT GMAIL SMTP?
 * Render free tier blocks outbound SMTP ports (465, 587).
 * Gmail SMTP will ALWAYS timeout on Render. This is a known limitation.
 * 
 * Resend uses HTTPS (port 443) which works on ALL hosting platforms.
...
// Hard-coded fallback so it works immediately after setup
const RESEND_API_KEY = process.env.RESEND_API_KEY || '';
const FROM_EMAIL = process.env.FROM_EMAIL || 'Amazon Clone <onboarding@resend.dev>';

const formatINR = (amount) => `₹${Number(amount).toLocaleString('en-IN')}`;

function getResendClient() {
  if (!RESEND_API_KEY) {
    console.error('❌ RESEND_API_KEY is not set! Get one free at https://resend.com');
    return null;
  }
...
async function sendOtpEmail(toEmail, otp, name) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to: [toEmail],
      subject: `${otp} is your Amazon Clone verification code`,
      html: `
        <!DOCTYPE html>
        <html>
...
async function sendOrderConfirmation({ toEmail, orderId, items, totalAmount, shippingAddress }) {
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to: [toEmail],
      subject: `Order Confirmed #${orderId.substring(0, 8)} — Amazon Clone`,
      html: `
        <!DOCTYPE html>
        <html>
        <head><meta charset="utf-8"></head>
    ...
    console.log(`📧 Order confirmation sent to ${toEmail} | id: ${data.id}`);
    return { success: true };
  } catch (err) {
    console.error('❌ sendOrderConfirmation exception:', err.message);
    return { success: false };
  }
}
...
async function sendOrderCancellation({ toEmail, orderId, items, totalAmount, reason }) {
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to: [toEmail],
      subject: `Order Cancelled #${orderId.substring(0, 8)} — Amazon Clone`,
      html: `
        <!DOCTYPE html>
        <html>
        <head><meta charset="utf-8"></head>
    ...
    if (error) {
      console.error('❌ Resend cancellation error:', JSON.stringify(error));
      return { success: false };
    }

    console.log(`📧 Cancellation email sent to ${toEmail} | id: ${data.id}`);
    return { success: true };
  } catch (err) {
    console.error('❌ sendOrderCancellation exception:', err.message);
    return { success: false };
  }
}
